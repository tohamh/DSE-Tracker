...
import { X, Search, Plus, Info, Calendar as CalendarIcon } from "lucide-react";
...
export default function AddTransactionModal({ 
  ...
  useEffect(() => {
    if (editingTransaction) {
      ...
      // Do not automatically set manual commission mode
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setActiveIndex(prev => (prev < filteredStocks.length - 1 ? prev + 1 : prev));
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div 
          ...
          className="relative bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors duration-300"
        >
          {/* Header */}
          <div className="p-2 md:p-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
            <h2 className="text-sm md:text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <Plus size={16} className="text-teal-500 md:w-5 md:h-5" />
              {editingTransaction ? "Edit Transaction" : "Add New Transaction"}
            </h2>
            <button onClick={onClose} className="p-1 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-all">
              <X size={16} />
            </button>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  type="date"
                  required
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg md:rounded-xl px-2 py-1.5 md:px-3 md:py-2 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all text-[12px] md:text-sm text-slate-800 dark:text-slate-100"
                  value={date}
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setType(newType);
                    if (newType === "Deposit" || newType === "Withdrawal" || newType === "Charge" || newType === "Dividend") {
                      setPortfolio("Investment");
              ...
              <div className="space-y-1 col-span-2">
                <label className="h-4 md:h-5 text-[10px] md:text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center">
                  Portfolio
                </label>
                  ...
                  {["Investment", "Trading"].map((p) => (
                    <label key={p} className="flex-1 cursor-pointer">
                      <input
                        ...
                        checked={portfolio === p}
                        onChange={() => setPortfolio(p as any)}
                      />
                      <div className={cn(
              ...
              <div className="space-y-1 relative col-span-2" ref={stockListRef}>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg md:rounded-xl px-2 py-1.5 md:px-3 md:py-2 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all text-[12px] md:text-sm text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
                  value={searchTerm}
              ...
              {(type === "Buy" || type === "Sell") && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between h-4 md:h-5">
                    <label className="text-[10px] md:text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center">Comm (৳)</label>
                    <button 
                      type="button"
                  ...
                  <input
                    ...
                    className={cn(
                      ...
                      isManualCommission ? "bg-white dark:bg-slate-900 border-amber-300 dark:border-amber-500/50 focus:ring-amber-500/20 text-slate-800 dark:text-slate-100" : "bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500"
                    )}
